...
- `mareforma.signing.verify_envelope(envelope, public_key, *, expected_payload_type=None)` accepts an explicit payload-type kwarg; callers that care about semantic boundaries (claim verifier rejecting an enrollment envelope and vice versa) pass the expected type. When omitted, any known mareforma payload type is accepted.
