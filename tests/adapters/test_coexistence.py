...
- :class:`TestSharedGraph` — three adapters write into one graph
  without colliding on predicate URIs.
- :class:`TestImportHygiene` — adapter imports must not register
  predicates as a side effect.
"""
...
class TestSharedGraph:
    def test_three_adapters_share_one_graph(self, tmp_path: Path):
