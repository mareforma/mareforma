...
A second independent-lineage run on the same proposition, on a distinct dataset
and a distinct model, lifts it to `CONVERGENT`:

```python
graph.assert_finding(prop, plan, est_beta, data_id="dataset_beta",
                     generated_by="analyst/model-b/lab_b")
graph.proposition_status(prop)["status"]   # "CONVERGENT"
```
