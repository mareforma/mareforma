...
Nothing stored on a *claim* records whether agents converged or whether a
human validated it (see [Trust](/concepts/trust)). The derived axes above are
what a reader reaches for, and a human's sign-off is a signed attestation rather
than a word on the row.
