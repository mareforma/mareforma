...
distinct-run heuristic: two supporting lines count as independent when they come
from different runs (`generated_by`) and different datasets (`data_id`). One run
contributes at most one independent support and one independent refute, so a
single run cannot reach `CORROBORATED` on its own. A finding may carry several
...
Status is a **versioned policy** (`status_policy@v2`) over durable stored
...
A second independent run on the same proposition, on a distinct dataset, lifts
it to `CORROBORATED`:
...
- **`Status`** (`UNTESTED`, `PRELIMINARY`, `CORROBORATED`, `REFUTED`,
