...
(who asserted it, when), so it appears in `query()` with a `classification`
...
`graph.proposition_status(prop)` returns both. It used to return a
`frame_status` key alongside them that echoed the answer's own word; that key is
gone, and `question_status` is what names the state of the question.

There used to be a **legacy layer** beside them: a stored `support_level` on
each *claim*, recording whether independent agents had converged and whether a
human had validated it. It is gone (see [Trust](/concepts/trust)). The derived
axes above are what a reader reaches for, and a human's sign-off is a signed
attestation rather than a word on the row.
