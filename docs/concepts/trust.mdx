...
Read the number, not the label: `REPLICATED` and `ESTABLISHED` resolve
...
<Note>
  ...
  a level. The levels below are still stored and still fire, but nothing public
  asks for one, and the effective-independence number above is the signal to
  read.
</Note>
