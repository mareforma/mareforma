...
<Note>
  ...
  one, and no field a writer could raise. If you have `REPLICATED` or
...
Two reasons, and both are the point of the project.

A single word never carries what a reader needs. "Two keys agreed" is a
convergence marker, not proof of independence, and a reader handed one word
takes it for the other. What you want to know is how many pairwise-distinct
(model, data, signer) checks stand behind a finding, and that is a count, not a
label.

And a word on a row is unsigned. Any process with write access could raise it,
so every read would have to re-derive whether the word was backed by signed
material before serving the row. If the derivation is what you trust, serve the
derivation and skip the word.
...
| Value | Meaning |
|---|---|
| `VALIDATED` | An enrolled validator signed off, and the envelope verifies on read |
| `VALIDATION_UNVERIFIED` | The row carries a validation envelope that did not verify on read |
| `UNRATIFIED` | Nobody has signed off on this claim |
