...
root. The support labels below still resolve this release, but as deprecated
one-release aliases: read the number, not the label.
...
<Note>
  ...
  described below, but the effective-independence number above is the signal to
  read.
</Note>
