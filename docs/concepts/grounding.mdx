...
the data the finding cites. So at bind time the verdict's cited set is cross-checked
against the finding's own citation: its `data_id` set plus any `data_source=`. A
`GROUNDED` whose cited set shares nothing with the finding downgrades to `OPAQUE`
with a signed reason, or raises `GroundingCitationMismatchError` when you pass
