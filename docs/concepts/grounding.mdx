...
- a cited path opened through an uninstrumented reader (`os.open`), or a cited
  C-runtime file (HDF5, netCDF, Arrow) whose bytes emit no PEP-578 event and were
  not read through a wrapped reader.
