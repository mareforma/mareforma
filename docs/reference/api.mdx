...
| `validator_type` | `str` | `"human"` | `"human"` or `"llm"`, the self-declared type recorded if this key auto-enrolls as the project's root validator. Ignored once a root exists. An LLM-typed validator can sign a validation envelope, and recording one on a claim is refused. |
...
Return the epistemic schema: the valid values a claim can hold, and their defaults.
...
s["defaults"]          # {'classification': 'INFERRED', 'status': 'open', ...}
```

There are no `support_levels` or `transitions` keys. They described the support
ladder, and the ladder is gone.

**Returns** `dict`: stable across patch releases within a major version.

---

## `mareforma.restore(project_root, *, claims_toml=None, rekor_log_pubkey_pem=None, enforce_rekor_policy=False, trust_unaccounted_backup=False)`
...
#  'unsigned_in_signed_mode': 0, 'verdict_chain_withheld': 0}
```
...
| `trust_unaccounted_backup` | `bool` | `False` | Restore a backup whose own completeness table does not match what the file holds, which is the path for an operator who edited it deliberately. It also covers a broken or short verdict chain and a `GROUNDED` observed axis arriving with nothing attesting it, rebuilding from the part that holds. It does NOT cover a backup written by a later format: this release cannot say what that file owes, so `format_ahead` is refused either way. |

**Returns** `dict` with `validators_restored`, `claims_restored`,
`unsigned_in_signed_mode`, and `verdict_chain_withheld`.
...
| `policy_violation` | A rebuilt row breaks the project's signed policy, such as an unwitnessed claim under a witnessing declaration. |
| `backup_unaccounted` | The file does not hold what its `[completeness]` table says it holds, so the rebuilt graph would be short rows with nothing recording they were there. Pass `trust_unaccounted_backup=True` to restore a file you edited deliberately. A backup written before the table reaches this check not at all. |
| `verdict_chain_broken` | The restored verdict chain does not account for the verdicts it carries, so a verdict was taken out of the backup. Pass `trust_unaccounted_backup=True` to restore it as it stands. |
| `verdict_chain_cut_short` | The backup says its own writer could not write the verdict chain in full, so the file is short of links the graph it came from still holds. Pass `trust_unaccounted_backup=True` to rebuild from the part that holds, which is a working graph shorter than the one backed up. |
| `grounding_unattested` | A claim carries a GROUNDED observed-grounding axis that nothing in the file attests, which is how an axis edited after the fact arrives. Only checked on a backup carrying the format stamp. Pass `trust_unaccounted_backup=True` to restore it as it stands. |
| `format_ahead` | The backup was written in a later format than this release understands, so what it owes cannot be checked here. The override does not apply; restore it with the release that wrote it. |
...
### `assert_claim(text, *, classification, generated_by, supports, contradicts, source_name, idempotency_key, status, artifact_hash, evidence, signer, predicate_payload, original_signature_bundle, grounding_sensor, observed_grounding, finding_record)`
...
| `generated_by` | `str \| None` | `"agent"` | Display label. Independence keys on the signing key, not this field. |
...
| `signer` | `object \| None` | `None` | Per-call override for the key the claim is signed with; `None` inherits the signer from `mareforma.open(key_path=...)`. The keyid it writes to `asserter_keyid` is the axis independence counts, so a host holding several keys asserts each lab's finding under its own. Signing does not enroll: only enrolled keys can `validate()`. |
...
**No side effect on other rows**: asserting a claim writes that claim. Claims sharing an upstream under distinct signing keys are separate lines the independence axis counts on read.
...
**Not a convergence mechanism**: two agents reaching the same conclusion stay two lines by each signing under its own key while citing the same upstream. Collapsing them into one row would erase the second contribution.

---

### `query(text=None, *, classification=None, limit=20, include_invalidated=False, refutation_filter=None)`

Query claims ordered by recency (descending).

```python
results = graph.query("cell type A", classification="ANALYTICAL")
for r in results:
    print(r["text"], r["classification"], r["validator_reputation"])
```
...
one root of trust. It is a disclosure, not a Sybil guard.

**Raises** `ValueError` if `classification` is invalid.

Rows carrying signed material are re-verified on read: a row whose signature fails to verify is excluded from the result. Never raises.


---

### `search(query, *, classification=None, limit=20, include_invalidated=False)`
...
```python
hits = graph.search("dopaminergic", classification="ANALYTICAL")
...
expression; it is the positional `query()` takes as `text`.
`classification`, `limit` and `include_invalidated`
...
**No side effect on the claims it names**: the verdict is a signed record of
one party corroborating another, and it lifts nothing.
...
is the number of claims whose validation envelope was
signed by that keyid. Validators that have signed none appear with
...
### `query_for_llm(text=None, *, classification=None, limit=20)`
...
```python
findings = graph.query_for_llm("topic X", classification="ANALYTICAL")
...
**Returns** `dict | None`: `None` if not found. Same field shape as `query()`. A row carrying signed material is re-verified on read; if its signature fails to verify the dict is returned with `verified=False` rather than excluded. Never raises.
...
A status change is editorial: it produces no signed envelope, names no validator keyid, and nothing in mareforma records who pulled the lever. Any process with write access to the database can flip a validated claim to `retracted`. For a retraction another party can verify, assert a new claim with `contradicts=[claim_id]` under a validator key; that leaves a signed envelope and a contradiction verdict `restore` re-verifies.
...
**Returns** `dict` with keys `state`, `reason`, `signal`. `state` is `clean` | `contradicted` | `contested` | `retracted`. `signal` names how strong the evidence behind that state is. A `contested` state reached editorially and one reached by signed verdict read the same in `state` and differ only here.

| `signal` | What it means |
|---|---|
| `signed-verdict` | The invalidation column is set and a contradiction verdict that verifies backs it. The only reading that survives a hostile writer. |
...
| `unbacked-invalidation` | The column is set and no verifying verdict names this claim. Somebody wrote the column. |
| `suppressed-verdict` | A verifying verdict invalidates this claim and the column is clear. Somebody erased it from every read surface. |
| `unverifiable-verdict` | Verdicts naming this claim exist and do not check out. Planted rows, not weak evidence. |
| `replay-unavailable` | The replay could not run, so nothing signed stands behind the column either way. |

The last four are `REPLAY_TAMPER_SIGNALS`: the column and the signed evidence
disagree. For the first three of those the state stays `contradicted` whichever
way the disagreement runs, because handing back a suppressed claim as clean
finishes the job for whoever wrote the column, and leaving a claim clean
because the column is clear finishes it for whoever cleared it.
`replay-unavailable` follows the column instead, since a replay that did not
run is not evidence that one would have found something.

`mareforma.replay_contradictions(conn, claim_id)` is the replay itself, for a
caller who wants the verdicts rather than the verdict. It fetches every
contradiction verdict naming the claim on either side and holds each against
its issuer: enrolled validator, signature verifying over the DSSE PAE rebuilt
from the stored columns, the same bar the recording path applies. A verdict
that fails is not weaker evidence, it is a row somebody planted, and it is
reported rather than skipped.

The graph-bound method replays the verdicts. The module-level
`mareforma.refutation_status(row)` reads the column and warns, because
`t_invalid` carries no write guard: one `UPDATE` fabricates a contradiction and
one erases a real one. Pass a connection as its second argument, or use
`graph.refutation_status(claim_id)`, to get an answer checked against signed
material. The row-only form is deprecated and removed in a future release.
...
The graph must have a loaded signer (from `mareforma bootstrap` or `mareforma.open(key_path=...)`) AND that key must be enrolled in the project's `validators` table. The first key opened against a fresh graph auto-enrolls as the root validator. The validation event itself is signed: a DSSE-style envelope binding `(claim_id, validator_keyid, validated_at, evidence_seen)` is persisted to the row's `validation_signature` column, so the validation is independently verifiable.
...
**Raises**
- `ClaimNotFoundError`: if the claim does not exist
- `ValueError`: if no signer is loaded, if the claim is not open, if it already carries a validation, or if the loaded signer is not an enrolled validator
...
- `LLMValidatorPromotionError`: if the loaded signer is enrolled with `validator_type='llm'`. LLM-typed validators sign validation envelopes, and recording one is refused; a human-typed validator has to make the call
...
- `InvalidValidationEnvelopeError`: if the signed envelope fails a structural or cryptographic gate (malformed payload, wrong `payloadType`, signer not enrolled, signature does not verify, or a payload field disagreeing with the row being validated)
...
A "healthy" graph has zeros across the three drift counters (`unsigned_claims`, `unresolved_claims`, `dangling_supports`). Non-zero values don't by themselves indicate a defect; they indicate something the operator should look at.

**Returns** `dict[str, int]` with the five keys above.
...
### Contested rows a read served

There is no counter property for this one, because these rows are not held
back. A claim whose `t_invalid` column disagrees with the signed contradiction
verdicts is still returned by `query()` and `search()` unless you asked for the
clean filter, so the disclosure goes to the `mareforma` logger and to
`.mareforma/health.jsonl` rather than to a count of what you did not get.

Two warnings, and they are different news. `Read withheld N claim(s) whose
contradiction record the signed verdicts do not support` fires when the clean
filter dropped a row. `Read served N claim(s) whose contradiction record the
signed verdicts contradict` fires for the other direction, where a real
invalidation was cleared and the row passed the filter and reached you looking
ordinary. The second is the one worth acting on: `t_invalid` carries no write
guard, so one `UPDATE` takes a contradiction off every listing.

Served rows are appended to `.mareforma/health.jsonl` as
`read_contested_served` with outcome `degraded`, under the same rate limit as
`read_verify_excluded`. They are counted apart from the exclusion counters
above on purpose: those rows are missing from your list, these were handed to
you. Name the affected claims with `mareforma verify`.
...
The independence count already refuses to count a dangling reference; this helper is for auditing integrity, not for blocking writes.
...
Mareforma uses this same classification for cycle detection, dangling-reference audit, and JSON-LD export. Exposed publicly so callers can introspect what it sees for any candidate list before insertion.
...
| `validator_type` | `str` | `"human"` | `"human"` or `"llm"`. Self-declared honesty signal, bound into the signed enrollment envelope. LLM-typed validators may sign validation envelopes, and recording one on a claim is refused: `validate()` turns them away at the core with `LLMValidatorPromotionError`. |
...
| Name | Type | Default | Description |
|---|---|---|---|
| `generated_by` | `str` | `"agent"` | Agent identifier baked into the closure, carried as a display label on each claim. Independence is the signing key (`asserter_keyid`), not this field; give each independent agent its own key. |
...
- `query_graph(topic) -> str`: routes through `query_for_llm`. Returns a JSON string of matching claims with free-text fields sanitized and wrapped in `<untrusted_data>...</untrusted_data>`.
...
| `IllegalStateTransitionError` | `mareforma.db` | DB triggers | A write violates a storage-layer state rule, such as reopening a retracted claim |
...
| `LLMValidatorPromotionError` | `mareforma.db` | `validate()` | The loaded signer is enrolled `validator_type='llm'`; a human-typed validator must sign the validation |
| `SelfValidationError` | `mareforma.db` | `validate()` | The loaded signer also signed the claim. A validation needs an external witness |
...
| `ProjectPolicyError` | `mareforma.db` | `require_rekor_witnessing()` | The project-policy declaration was refused: no signer loaded, the signer is not the project's single root validator, or a concurrent writer declared a rule the envelope in hand does not carry. Subclasses `ValueError` |
...
| `InvalidValidationEnvelopeError` | `mareforma.db` | `validate()`, `db.validate_claim()` | Validation envelope is structurally / cryptographically invalid: malformed JSON, wrong `payloadType`, signer not enrolled, signature does not verify against the claimed signer, or payload binds a different `claim_id` / `validator_keyid` / timestamp than the row being validated |
| `UnverifiedClaimError` | `mareforma.db` | `JSONLDExporter.export()`, `build_prov_o()`, `build_crate()` | The export would publish a claim carrying signed material that no longer re-verifies. Names the claim ids; run `mareforma verify <claim_id>` for the detail |
...
| `NoPerturbationsError` | `mareforma.observe` | `perturbation_oracle()` | `perturb` was an empty sequence, so there is no null to run. A caller handing the oracle nothing to measure is a bug in the call, never a `NOT_TESTED` row a report would count. Subclasses `ValueError` |
| `BundleVerificationError` | `mareforma.export_bundle` | `mareforma verify` | Signed export bundle failed a check: DSSE signature, per-claim subject digest, per-claim asserter signature, validator chain, or a validation envelope a node carries |
...
Schema-ahead message (raised when `open_db()` finds a graph.db whose `user_version` is **higher** than mareforma's `_SCHEMA_VERSION`): `"graph.db has user_version=N, which is ahead of the user_version=M this mareforma understands. It was written by a newer release and may carry schema this one does not know how to read, so opening it could report on a graph it is misreading. Nothing is wrong with the file. Upgrade mareforma to the version that wrote it. Do not delete graph.db: it holds the chain and every signature, and claims.toml cannot reconstruct them."` A graph at a **lower** `user_version` is not an error: it is migrated on the open that meets it.
...
`query()` with a `classification` like any other claim.
...
question, either consistent or divided), `status_policy`, `lines_skipped`, the count of
...
The observed code shares your interpreter, which is what makes its reads visible
at all. A verdict therefore grades a pipeline that does not attack its observer:
code written to defeat the observation could fabricate what is recorded. The
verdict attests what the process saw, not the honesty of the code it watched.
...
### `perturbation_oracle(run_fn, base_input, perturb=None, *, repeats=1, metric=None, effect_threshold=0.0, noise_multiplier=3.0, multiplicity=1, thin_sigma_guard=False, determinism_rtol=1e-06, determinism_atol=0.0, quantity_class='CONTENT_DEPENDENT', on_progress=None)`
...
```python
from mareforma.exporters import JSONLDExporter
```
...
| Name | What it is |
|---|---|
| `OracleInfluence` | the verdict: `INFLUENCED`, `NOT_INFLUENCED`, `UNDECIDABLE`, `NOT_TESTED` |
| `NotTestedReason` | why no verdict was produced: `unsupported-shape`, `null-construction-failed`, `target-failed`, `crashed-under-null`, `unreducible-value`, `non-finite-value` |
| `NullOutcome` | how one null was classified: `MOVED`, `AMBIGUOUS`, `BELOW_DOMAIN_FLOOR`, `FLAT` |
| `QuantityClass` | what kind of quantity the finding reports: `CONTENT_DEPENDENT` (the default), `LENGTH_INVARIANT`, `DESIGN_FIXED` |
| `Scramble` / `ScrambleFamily` | one null, and the family that will run plus the ones the input ruled out |
| `scramble_family(x)` | build that family from a value's shape, or `None` when no family fits |
| `THREAT_MODEL_STATEMENT` | the standing bound every verdict is made under |
| `NoPerturbationsError` | raised when `perturb` is an empty sequence |

The derived family differs by shape, and the names appear in `scramble_names`,
`dropped_nulls` and `blind_spot_line()`:

| Shape | Nulls |
|---|---|
| a real scalar | `zeroed`, `negated`, `offset`, `scaled`, `replaced`, all value replacements; a scalar has no marginal-preserving null |
| a mapping of scalars | `zeroed`, `constant`, `permuted` |
| a sequence of scalars | `zeroed`, `constant`, `permuted`, `reversed` |

A null identical to the base cannot perturb anything, so it is dropped and named
in `dropped_nulls` rather than run.

Declaring a quantity class only ever turns a hollow verdict into an undecidable
one, so a target cannot buy itself a clean read by mislabelling what it
reports. Every null in the sequence and mapping families returns a container of
the same length, so a quantity determined by length alone comes out flat under
all of them by construction. A reported sample size, a cell count or a
degrees-of-freedom figure is unmoved for a reason that has nothing to do with
the pipeline, and reading that as `NOT_INFLUENCED` turns the most common
integers in any write-up into an accusation. `LENGTH_INVARIANT` and
`DESIGN_FIXED` route those to `UNDECIDABLE` instead.

### `provenance_diff(reported, released) -> ProvenanceReport`

The second detector, and it asks a cheaper question than the oracle: does the
number in the write-up appear in the outputs the work released. It runs
nothing, opens nothing, and needs no key, so it reaches targets the oracle
declines. The oracle needs a target that re-runs cheaply and near
deterministically; most work is not that.

```python
from mareforma.observe import ReportedValue, ReleasedValue, provenance_diff

report = provenance_diff(
    reported=[ReportedValue(slot="auc", value=0.91, as_written="0.91", where="abstract")],
    released=[ReleasedValue(slot="auc", value=0.9104, source="results.csv")],
)
report.findings[0].verdict   # NumericProvenance.TRACEABLE
```

Both sets are the caller's. Pulling numbers out of a write-up and collecting
the outputs of a run differ per corpus, and building either one in would fit
the detector to one corpus and no other.

**Returns** a `ProvenanceReport` with `findings`, `inconsistent_slots` and
`released_slots`. Each `ProvenanceFinding` carries the `reported` value, a
`verdict`, a `reason`, and the `matched_sources` it was found in.

| `NumericProvenance` | What it means |
|---|---|
| `TRACEABLE` | A released output for that slot carries this value, at the precision it was written to. |
| `UNTRACEABLE` | Nothing released covers that slot. A gap in what was handed in, not an accusation. |
| `CONTRADICTED` | The outputs cover the slot and none carry the value, or a stated direction of change disagrees with the numbers it is derived from. |

`inconsistent_slots` is the check no per-number verdict can make. When one
quantity is stated more than once with values that cannot all be right, each
may trace to some run on its own while the write-up contradicts itself, so it
is reported per slot rather than folded into the per-number verdicts.
Restating a rounded value is not a conflict, so the comparison uses the coarser
of the two precisions.

What a `TRACEABLE` number is not: evidence the run was correct, that the output
came from the pipeline described, or that the right output was selected. It
appears in the outputs supplied, and the report says only that.

### `cross_tabulate(influence, provenance) -> CrossTab`

Place one finding on both detectors at once. Neither number means much alone: a
hollow rate cannot tell a finding that ignores its data from one whose data was
never in the outputs, and a provenance rate cannot tell a number that is merely
absent from one that nothing computed. The two middle cells exist only because
both arms ran on the same finding.

| `CrossTab` | Influence arm | Provenance arm |
|---|---|---|
| `AGREE` | depends on its data | the number is there |
| `AGREE_ABSENT` | no dependence | no coverage |
| `TENSION` | the data matters | the number is not in the outputs |
| `CONSTRUCT_DIFFERENCE` | no dependence | the number is there. The cited-but-hollow case |
| `CONVERGENT_FAILURE` | no dependence | the outputs contradict the number. The worst cell |
| `OBSERVER_BLIND` | the oracle could not see the target | any |
| `INCONCLUSIVE` | either arm declined to call it | |

Pass `None` for either arm when that detector did not run. Both arms accept
their enum or its string form, and an unrecognised token raises rather than
landing the finding in a cell it does not belong in. On real statistics
`INCONCLUSIVE` is the modal answer, not a rare edge.
...
```python
from mareforma.selfcheck import run_selfcheck

for outcome in run_selfcheck(tmp_path):
    print(outcome.name, outcome.caught, outcome.observed)
```
...
stored record, or `None`). A finding whose verdict is not `GROUNDED` reads
`UNGROUNDED` or `OPAQUE` on the trust map's grounding axis and gates nothing
else; a claim asserted without a verdict is
