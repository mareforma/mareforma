...
### `query(text=None, *, classification=None, limit=20, include_unverified=False, include_invalidated=False, refutation_filter=None)`
...
The `support_level` result key is the legacy stored promotion ladder. No public read filters on it any more and the `REPLICATED` / `ESTABLISHED` module labels are removed. To read trust, use the derived axes `graph.proposition_status(prop)` returns: `status` (the answer, per `content_id`) and `question_status` (the question, per `frame_id`).

---

### `search(query, *, classification=None, limit=20, include_unverified=False, include_invalidated=False)`
...
A PRELIMINARY claim whose generator key is not in the validators table is dropped from an enumerating read unless you pass `include_unverified=True`. Unlike the verify exclusions above, a flag does bring these back: they are not tampered rows, they are rows the default read does not vouch for. Without the count, a project whose claims were written under an unenrolled key answers a query with an empty list, and a caller reads "there is nothing here" for a record that is not empty.
