...
### `assert_claim(text, *, classification, generated_by, supports, contradicts, source_name, idempotency_key, status, artifact_hash, evidence, seed, predicate_payload, original_signature_bundle, grounding_sensor, observed_grounding)`
...
| `grounding_sensor` | `object \| None` | `None` | Optional sensor that computes the observed-grounding verdict for the claim. |
| `observed_grounding` | `dict \| None` | `None` | Pre-computed observed-grounding record, stored in the queryable `observed_grounding` column. |
...
`matched_identifier`, `version`, `reads_seen`, `opens_detected`.
...
### `perturbation_oracle(run_fn, base_input, perturb, *, repeats=1, metric=None, effect_threshold=0.0, noise_multiplier=3.0)`
...
`opaque_dominates(threshold=0.5)`.
