...
- `EvidenceCitationError` — if any `evidence_seen` entry is not a strict-v4 UUID, does not point to an existing claim, or post-dates `validated_at`
...
| `EvidenceCitationError` | `mareforma.db` | `validate()` with `evidence_seen=...` | A cited entry isn't a strict-v4 UUID, points to a non-existent claim, or post-dates `validated_at` |
