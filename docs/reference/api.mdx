...
```python
graph.read_unverified_exclusions  # 0
```

A claim whose generator key is not in the validators table is served, carrying `generator_enrolled=False`. It used to be dropped from an enumerating read, and converging lifted it back into view; nothing converges now, so that rule would have answered every query on a project that enrols no validator with an empty list, and a caller would read "there is nothing here" for a record that is not empty.
