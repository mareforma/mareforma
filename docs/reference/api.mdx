...
```python
from mareforma.derivation import (
    derive_classification, verify_classification,
    extract_source_profile, extract_templates,
    DERIVATION_VERSION,
)

result = derive_classification(source_code, runtime_logs)
# ClassificationResult with .classification + .evidence
```

Source-profile extraction requires `pip install mareforma[derivation]` (`tree_sitter` + `tree_sitter_python`); log-template extraction (Drain parser) is pure stdlib.
...
member), `reason` (str), `cited_sources`, `reads`, `seams`, `matched_identifier`,
`version`, `reads_seen`, `opens_detected`.
...
- `receipt()` returns the full canonicalizable evidence; `receipt_digest()` is its
  `sha256:` digest.
- `to_signed_dict()` is the compact `{version, grounding, reason, receipt_digest}`
  record bound into the signed statement. mareforma binds the digest, not the
  receipt, so a caller that keeps the receipt out of band can detect mutation.
