...
| `evidence` | `EvidenceVector \| dict \| None` | `None` | GRADE evidence vector for the claim, denormalised into the `ev_*` columns and stored as `evidence_json`. |
...
Used internally by the v0.3.3 CLI subcommands (`ingest`, `ask`, `narrative`) when `--db` points outside `.mareforma/`.
...
(`"status_policy@v3"`), independent of the package version.
