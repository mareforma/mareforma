...
`t_invalid`, `predicate_payload`, `original_signature_bundle`,
`observed_grounding`, `created_at`, `updated_at`. `observed_grounding` is the
stored grounding verdict (the queryable denormalisation of the signed
predicate's grounding record). Plus two reputation projections computed at query time:
...
For ESTABLISHED rows, each result also carries `single_trust_domain: bool`, a disclosure of whether all validators trace to one root of trust. It is a disclosure, not a Sybil guard.
