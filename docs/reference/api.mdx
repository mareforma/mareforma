...
|---|---|---|
| `content_id()` | `str` | `sha256` over normalized `(subject, relation, object, scope, direction, magnitude)` — the *answer*. Same truth conditions → same id. |
...
**Returns** `dict` — `finding_id`, `content_id`, `plan_id`, `claim_id`,
...
**Returns** `dict` — `content_id`, `frame_id`, `direction`, `status`,
