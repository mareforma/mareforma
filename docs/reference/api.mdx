...
`not_tested_dominates(threshold=0.5)`, and `influenced_fraction` over the
`resolved` edges only (NOT_TESTED is excluded from the denominator). The unit is
the edge, so a finding citing several sources contributes several edges; the
inference unit is the run, so `distinct_runs` rides alongside the rate. A receipts
file with no `influence` record yields an empty arm rather than a fabricated one.
