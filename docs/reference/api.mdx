...
`lines_skipped`, the count of this proposition's evidence lines that no
longer reconstruct into a gateable bearing and were therefore left out of the
counts, and `post_hoc`. A dropped refutation reads as consensus, so read
`lines_skipped` next to the two counts.
