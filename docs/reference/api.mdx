...
The count is taken only when a read comes back short, since a full page already says `has_more`, and it is bounded by the same scan ceiling as the read it describes. The MCP `query_claims` and `search_claims` tools report the per-call number as `unverified_excluded`.
...
classification the verdict was routed on (`MOVED` / `AMBIGUOUS` / `FLAT`), which
`flat_nulls`, `ambiguous_nulls` and `blind_spot_line()` all read rather than
re-deriving.
