...
- `read_coverage_fraction()` is `reads_seen / opens_detected`, or `None` when
  nothing was opened.
...
read as influenced by its own jitter. `reconcile(grounding, influence)` relates
the observer's flow verdict to the oracle's influence verdict.
