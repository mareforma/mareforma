...
- `Contrast(control_type=ControlType.NEGATIVE)` and
  ...
  are the one-line evidence tree. `data_id` is the distinct-artifact key the
  independence heuristic counts over.
...
has ≥1 independent supporting line, else `CONSISTENT`. `STATUS_POLICY` is the
policy stamp (`"status_policy@v1"`), independent of the package version.
...
#### `submit_finding(proposition, prediction, estimate, *, data_id, generated_by=None, control_type=None, modality=None, provenance_id=None, design_type=None, code_ref=None, idempotency_key=None) -> dict`
...
denormalised metadata. A finding already recorded for `(content_id, data_id)`
under a *different* `plan_id` raises `FindingPlanForkError` rather than silently
returning the prior bearing. The existence check and the writes run in one
transaction.

#### `assert_finding(proposition, prediction, estimate, *, data_id, generated_by=None, control_type=None, modality=None, provenance_id=None, design_type=None, code_ref=None, idempotency_key=None) -> dict`

Record a finding: validate the inputs, compute the bearing, write a signed claim
as the attestation, persist the single-line evidence tree, and derive the
...
`bearing` (`{"direction", "significant"}`), `status`, `idempotent` (bool), and
`proposition_status` (the full view below).

Idempotent on `(content_id, data_id)`: re-asserting the same finding on the same
dataset returns the prior finding rather than double-counting it. All validation
runs **before** the signed claim is written, so a rejected finding never leaves
an orphan claim. Raises `NonFalsifiablePropositionError` /
