...
| `signature_bundle` | TEXT | `NULL` | Yes | DSSE-style Ed25519 envelope over `canonical_payload` |
| `transparency_logged` | INTEGER | `1` | No | `1` when the claim has been (or did not need to be) submitted to Sigstore-Rekor. Blocks REPLICATED until `refresh_unsigned()` flips it |
...
| `prev_hash` | TEXT | `NULL` | Yes | Append-only chain link `sha256(prev_chain_link \|\| canonical_payload)`. UNIQUE partial index catches branched chains |
...
Removal is intentionally unsupported in v0.3.x — validator history is append-only.
...
No in-place migrations during v0.3.x. Adding a column, index, or
