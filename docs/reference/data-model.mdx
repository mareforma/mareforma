...
| `unresolved` | INTEGER | `0` | No | Legacy quarantine flag, retained inert this release (always `0`). The column, its index, and `list_unresolved_claims` stay in the schema so restore and promotion counting round-trip unchanged |
...
| `idx_claims_unresolved` | `unresolved` | Non-unique | Legacy index on the inert `unresolved` flag |
...
A root-signed, single-row (`id = 1`) declaration of project-wide trust policy. Today it carries `rekor_required`: once set, findings must be witnessed by the transparency log before they can converge (this backs `require_rekor_witnessing`). The signed `envelope` is the authority; the flat columns are a denormalized read cache. Restore verifies the envelope against the enrolled root before enforcing.
...
| `model_lineage` | TEXT | JSON record `{tier, model_id, family_root, provider, version, method, decoding}` of the model/method observed at the call boundary. Tier mirrors the `data_id` axis: `COMPUTED` (body-parse at the socket seam), `PROXY` (producer-declared), `UNVERIFIABLE` (a fine-tune / alias / wrapper whose base is not declarable). Identity only. NULL on every line authored without an observed model call (including rows that predate this column) |
