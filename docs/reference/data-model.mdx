...
| `support_level` | TEXT | `PRELIMINARY` | No | The stored promotion ladder: `PRELIMINARY` / `REPLICATED` / `ESTABLISHED`. Transitions enforced by BEFORE triggers. REPLICATED fires when ≥2 claims signed by distinct keys (`asserter_keyid`) cite a shared ESTABLISHED upstream. This is the legacy per-claim axis; its `REPLICATED` / `ESTABLISHED` public labels are deprecated for v0.4.0. Read trust off the derived `Status` and `question_status` axes (see below) |
...
| `generated_by` | TEXT | `agent` | No | Agent identifier; display label only. The REPLICATED independence axis is the signing key, see `asserter_keyid` |
| `asserter_keyid` | TEXT | `NULL` | Yes | Denormalized signer keyid from the `signature_bundle`. The REPLICATED independence axis: two claims signed by distinct keyids count as independent; an unsigned claim (NULL) cannot reach REPLICATED |
...
| `unresolved` | INTEGER | `0` | No | Legacy quarantine flag. No write path sets it, but restore replays `unresolved = true` from an older `claims.toml`, and a set flag holds the claim out of REPLICATED promotion with no error raised. `mareforma.db.list_unresolved_claims(conn)` lists the flagged rows; `mareforma.db.mark_claim_resolved(conn, root, claim_id)` clears one and re-checks convergence in the same transaction. `EpistemicGraph.update_claim` is not a substitute: it refuses `supports`/`contradicts` edits on signed claims |
...
| `idx_claims_asserter_keyid` | `asserter_keyid` | Non-unique (partial) | `WHERE asserter_keyid IS NOT NULL`: the REPLICATED independence axis |
| `idx_claims_convergence_retry` | `claim_id` | Non-unique (partial) | `WHERE convergence_retry_needed = 1`: keeps `refresh_convergence()` O(retry-pending) rather than O(N) |
...
Every code is a static suffix, not a from/to pair: `RAISE()` cannot concatenate a column value below SQLite 3.46. Python translates any `mareforma:state:` code to `IllegalStateTransitionError`, carrying the suffix.
...
The per-project set of enrolled public keys. Only `human`-typed rows can promote claims to `ESTABLISHED`.
...
| `validator_type` | TEXT | `'human'` (default) or `'llm'`. A self-declared honesty signal bound into the signed enrollment envelope, so a post-hoc UPDATE breaks chain verification. `llm` rows may sign validation envelopes but cannot promote a claim past `REPLICATED` and cannot issue contradictions |
...
| `strict_promotion_required` | INTEGER | `1` when a converging pair must carry `artifact_hash` on both sides |
...
| `strict_promotion_declared_at` | TEXT | UTC ISO 8601: when `strict_promotion_required` was first declared, NULL until then |
...
structured findings. They are **additive**: `CREATE TABLE IF NOT EXISTS`, no
migration, `user_version` stays `1`. A finding is an evidence tree
...
read trust off; the stored `support_level` ladder above is the deprecated legacy
axis.
...
Somebody who drops the guarded table, the seen store and the census together
gets all three rebuilt empty on the next open with nothing recorded, and no
scheme confined to one file the attacker can write to will change that. What
...
A migration runner exists and its registry is empty. Nothing in this release
moves `user_version`, so no graph migrates. When a release does add a route, the
schema change and the version bump commit in one transaction or neither does, so
a crash leaves the graph at the version it started from with every claim,
signature and chain link intact.
