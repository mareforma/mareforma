...
| `foreign_keys` | `ON` | SQLite default is OFF. Every table carrying a `REFERENCES` clause points at `validators(keyid)` or `claims(claim_id)`; without this PRAGMA those constraints are advisory and a direct-SQL INSERT naming a fabricated keyid or a claim that does not exist succeeds in silence |
