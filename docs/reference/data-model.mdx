...
One independent line of evidence. `data_id` is the distinct-artifact key the independence heuristic counts over.

| Field | Type | Notes |
|---|---|---|
| `line_id` | TEXT (PK) | UUID |
| `finding_id` | TEXT | FK to `findings(finding_id)`. Indexed (`idx_line_finding`) |
| `data_id` | TEXT | The independence key; two lines are independent iff this differs. Indexed (`idx_line_data`) |
...
independent supporting / refuting line counts (`status_policy@v1`), so improving
the rule later is a new policy over the same data, not a migration.
