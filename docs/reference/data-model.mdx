...
| `content_digest` | TEXT | SHA-256 hex of canonicalized registry metadata (title, year, container-title, author family names). NULL when the row carries only a HEAD-check result. `find_drifted_dois` compares a fresh fetch against this to detect post-publication corrections or retractions |
...
PROV-O Activity rows for ambient Claude Code tool calls, written by `mareforma.hooks.agent_hook`. High-volume, low-semantic-density data: most rows never escalate into signed claims, so they live outside the signed envelope chain.

| Field | Type | Description |
|---|---|---|
| `id` | INTEGER (PK) | AUTOINCREMENT |
| `session_id` | TEXT | Agent session identifier (nullable) |
| `tool_name` | TEXT | The tool that was invoked |
| `tool_input` | TEXT | Serialized tool input |
| `started_at` | TEXT | UTC ISO 8601 |
| `prov_type` | TEXT | PROV-O type, default `prov:Activity` |
