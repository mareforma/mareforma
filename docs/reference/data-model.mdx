...
distinct signing key, with `data_id` as a guard so the same dataset is not
counted twice.
...
independent supporting / refuting line counts, counted by distinct signing key
(`status_policy@v3`), so improving the rule later is a new policy over the same
