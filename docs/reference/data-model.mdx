...
| `support_level` | TEXT | `PRELIMINARY` | No | Graph-derived trust: `PRELIMINARY` / `REPLICATED` / `ESTABLISHED`. Transitions enforced by BEFORE triggers. REPLICATED fires when ≥2 claims signed by distinct keys (`asserter_keyid`) cite a shared ESTABLISHED upstream |
...
The trust layer (see [Findings](/concepts/findings)) adds six tables for
...
None of these columns is signed, so `restore` re-derives the row from the attestation: the claim's text renders plan, replacement and reason, and a row its claim does not render fails the restore with `kind='claim_unverified'`. Resolution on the read is reached only from a plan whose own rule cannot be run, and those lines count zero as they stand, so a planted retirement can recover a dropped line but never drop a counted one.
...
The `Status` of a proposition is not stored. It is derived on read from the
independent supporting / refuting line counts, counted by pairwise-distinct
model, dataset, and signer (`status_policy@v4`), so improving the rule later is
a new policy over the same data, not a migration.
