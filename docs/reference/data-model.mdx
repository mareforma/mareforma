...
| `foreign_keys` | `ON` | SQLite default is OFF. The `replication_verdicts` and `contradiction_verdicts` tables have FK references to `validators(keyid)` and `claims(claim_id)` — without this PRAGMA, those constraints would be advisory and a direct-SQL INSERT with a fabricated keyid would silently succeed |
