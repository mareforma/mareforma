...
| 🟡 YELLOW | Claims exist but all are PRELIMINARY, or a promoted claim does not re-verify on read |
| 🟢 GREEN | At least one REPLICATED or ESTABLISHED claim, and every promoted claim re-verifies |
...
Auditor mode verifies against enrolled validator pubkeys only, so a signed claim whose signer is **not an enrolled validator** cannot be authenticated: the claim's binding is all that could be checked, never its signature. `verify` exits **1** on such a claim (tamper or binding violation), not 0. A CI gate built on `verify` therefore no longer passes a claim carrying an all-zero signature under a keyid that was never enrolled.
