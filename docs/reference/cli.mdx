...
`query_claims` and `search_claims` serve at most 200 rows, 20 by default. Every page carries `count`, the `limit` actually served, and `has_more`: a short page is never mistaken for the whole record, whether it was shortened by the cap or by the default. A `limit` that was not honoured (above the cap, below one row, or not a number) comes back as `limit_requested` rather than being silently swapped.
