...
| `proposition_status` | `{found, status}` for a content id. `status` is the whole derived view: `status`, `question_status`, `independent_support`, `independent_refute`, `lines_skipped`, `post_hoc`, `frame_status`, `status_policy`. `null` when nothing resolves to that id |
