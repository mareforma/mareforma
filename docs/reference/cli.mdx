...
| `mareforma ingest` | Extract claim drafts from a paper abstract |
| `mareforma ask` | BM25 search over ingested literature claims |
| `mareforma narrative` | Export literature claims as a Markdown narrative |
...
Available commands: `bootstrap`, `key`, `validator`, `claim`, `status`, `activity`, `export`, `verify`, `map`, `diagnose`, `observe`, `measure`, `restore`, `ingest`, `ask`, `narrative`.
...
Read a paper abstract from disk, extract claim drafts, and write them to the project's `literature_claims` table (separate from the signed `claims` table, since most drafts stay drafts pending review).
...
| `--model MODEL` | `claude-opus-4-8` | Anthropic model name when `--llm` is set. |
...
mareforma ingest preprint.txt --llm --model claude-opus-4-8
...
FTS5 BM25 search over ingested literature claims. Input is sanitized so hyphens, parens, and asterisks are treated as literals; embedded double quotes are escaped per the FTS5 spec.
...
Export all ingested literature claims as a Markdown narrative grouped by source document, with structural and polarity-heuristic contradictions flagged inline.
