...
Also demonstrates a spurious convergence on the stored `support_level` ladder
(a `REPLICATED` label, deprecated for v0.4.0): two INFERRED claims repeating the
same LLM prior, and how to detect it from `classification` and the derived
independence axis rather than trusting the stored label.
