...
Also demonstrates a spurious convergence: two INFERRED claims repeating the
same LLM prior, and how to tell that from `classification` and the derived
independence axis rather than from the fact that two agents agreed.
