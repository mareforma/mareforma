...
@tool
def record_claim(text: str, classification: str, supports: list[str]) -> str:
...
rule. The two lines are distinct runs (`generated_by`) on distinct datasets
(`data_id`). On an unsigned graph, independence is counted on that run axis, so
the proposition reaches `CORROBORATED`. When findings are signed, independence
counts distinct *authenticated signers* instead: two analysts each write
through a graph opened with their own key, so a single signer cannot
self-corroborate. See [Findings](/concepts/findings).
