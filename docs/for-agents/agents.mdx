...
<Warning>
  `REPLICATED` / `ESTABLISHED` are removed public labels: reading
  `mareforma.REPLICATED` raises `AttributeError`, and no public read filters on
  a level. The levels below are still stored and still fire, but the lead independence
...
question, per `frame_id`: `consistent` or `divided`). It used to also return a
`frame_status` key; that key is gone.
...
signed verdict is one of the two things that earns a `REPLICATED` level, and
without them a bundle could not tell an honest promotion from an inflated one.
...
**What a REPLICATED display is checked against.** The same rule the graph
enforces, on either of its two paths: a replication verdict that verifies and
names the claim, or convergence, meaning a shared `ESTABLISHED` anchor carrying
a peer under a distinct asserter key whose artifact hash does not collapse into
this claim's. Both convergence terms have to hold on the same peer. Two claims
that each recorded no artifact hash still corroborate: a missing hash records
that no artifact was named, not that the same one was.
