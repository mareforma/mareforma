...
- **Capability URI constants** at `mareforma.predicate_types` (also re-exported at the top level): `TOOL_CALL_V1`, `CONTAINER_EXEC_V1`, `WORKSHOP_EVENT_V1`, `CODE_VARIATION_V1`, `HYPOTHESIS_V1`, `LITERATURE_INSIGHT_V1`, `SCIENCE_SKILL_V1`, `META_CLAIM_V1`, plus the substrate + wet-lab assay families. Use the constants instead of string literals so a typo fails at import.

## Substrate-derived classification (`mareforma.derivation`)
...
Source-profile extraction requires the `[derivation]` extra (`pip install mareforma[derivation]`) — gates `tree_sitter` + `tree_sitter_python`. Log-template extraction is pure stdlib and available unconditionally.
