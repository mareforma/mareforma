...
behind a finding). The older `REPLICATED` / `ESTABLISHED` support labels are
...
`REPLICATED` when two distinct signing keys converged, and to `ESTABLISHED` by a
human. It is gone, column and all, and nothing replaces it with another word.

<Warning>
  Reading `mareforma.REPLICATED` raises `AttributeError`, no read filters on a
  level, and no claim carries one. The signal to read is the
  effective-independence number on `mareforma map`: the count of
