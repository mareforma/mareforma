...
`REPLICATED` -> `ESTABLISHED`) is the legacy promotion ladder. It is still
stored, but no public read filters on it and the `REPLICATED` / `ESTABLISHED`
module labels are gone.
