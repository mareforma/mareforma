...
Usage
-----
    python 05_drug_target_provenance.py            # install + download + run
    ...
    python 05_drug_target_provenance.py --run      # run experiment only

Run from the examples/05_drug_target_provenance/ directory.
...
def stage_data():
    ...
    if not VENV_HF.exists():
        print(f"huggingface-hub CLI not found at {VENV_HF}. Run --install first.")
        sys.exit(1)
...
def stage_run():
    ...
    if not VENV_PY.exists():
        print("medea_env not found. Run --install first.")
        sys.exit(1)
    ...
    # Run both MEDEA forks and record findings via agent-native API
    run([str(VENV_PY), str(HERE / "run_experiment.py")], cwd=HERE, env=env)

    print("\nExperiment complete.")
    print(f"Claims: {HERE / 'claims.toml'}")
    print(f"Epistemic status: mareforma status")
